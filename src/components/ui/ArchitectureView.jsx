    ...
    <AnimatePresence>
      ...
      >
        <motion.div
          ...
          className="relative w-full max-w-5xl bg-[#0a0a0a] border border-slate-800 rounded-xl overflow-hidden shadow-2xl flex flex-col max-h-[90vh]"
        ...
        >
          ...
          <div className="relative flex-grow overflow-hidden bg-[#050505]">
            ...
            {activeTab === 'blueprint' ? (
              <div className="w-full h-full p-8 overflow-y-auto flex items-center justify-center min-h-[500px]">
                {/* Grid Background */}
                ...
                {/* Flow Container */}
                <div className="relative z-10 flex flex-wrap items-start justify-center gap-12 w-full max-w-4xl">
                  {['client', 'server', 'service', 'ai', 'database', 'code'].map((layer, layerIndex) => {
                    const layerNodes = nodes.filter(n => n.type === layer);
                    if (layerNodes.length === 0) return null;

                    return (
                      <div key={layer} className="flex flex-col gap-6 items-center justify-center min-w-[200px]">
                        {/* Layer Label */}
                ...
                {/* Legend / Tech Stack */}
                <div className="absolute bottom-6 left-6 flex flex-wrap gap-2 text-[10px] font-mono z-20">
                ...
                {project.codeSnippet ? (
                  <div className="relative">
                    <div className="absolute top-4 right-4 flex gap-2">
                      <button 
                        onClick={handleCopy}
                        className="px-3 py-1 bg-white/10 hover:bg-white/20 rounded text-xs font-mono text-slate-300 flex items-center gap-2 transition-colors"
                      >
                        {copied ? <Check size={12} className="text-green-400" /> : <Copy size={12} />}
                        {copied ? 'Copied!' : 'Copy'}
                      </button>
                      <div className="px-3 py-1 bg-white/10 rounded text-xs font-mono text-slate-300">
                    ...
                    <SyntaxHighlighter 
                      ...
                      style={vscDarkPlus}
                      customStyle={{ margin: 0, padding: '2rem', fontSize: '14px', lineHeight: '1.5', background: 'transparent' }}
